...
<div data-options="region:'north',split:true, border:false" height="60px">
    <div id="p" class="easyui-panel" title="管理提示" style="padding:5px;" iconCls="myicon-information">
        ...
        ${currentProj.title}，你一共有<strong>&nbsp;${count}&nbsp;</strong>个申请需要确认
    </div>
</div>
<div data-options="region:'center',split:true, border:false,title:'申请列表', iconCls:'myicon-table-go'">
    <table id="dg" class="easyui-datagrid"
        ...
        <tr>
            ...
            <th data-options="field:'teacherconf'" width="80">申请状态</th>
        ...
        <c:forEach items="${applys}" var="apply">
            ...
            <td><c:if test="${apply.teacherconf eq 0}">未确认</c:if>
                <c:if test="${apply.teacherconf eq 1}">已确认</c:if>
                <c:if test="${apply.teacherconf eq -1}">已拒绝</c:if>
            </td>
...
<script>
    ...
    function edit(applyid){
        d=$("#dlg").dialog({
            ...
            width: 860,
            height: 500,
            href:'${ctx}/console/gooddelay/edit-teacherConf?id='+applyid,
            maximizable:true,
            modal:true
        });
    }
